...
var MAIN_OBJECT = (function () {
   ...
   *************************************************************************/
  var getCookie, upload_data, startWS, sendNotification, notification_ws;
  ...
  upload_data = function (type_upload) {
    ...
    if (type_upload == "variablestationtype") {
      userFile = document.getElementById("variableStationTypes_csv_upload")
        .files[0];
      formData.append("csv_file", userFile);
      files_list.push(userFile["name"]);
    }
    ...
    $.ajax({
      ...
      error: function (xhr, errmsg, err) {
        $.notify(
          `There was an error while uploading the data to the MCH database`,
          "error"
        );
  ...
  $(function () {
    ...
    $("#variableStationTypes_csv_upload_button").click(function () {
      var files = document.getElementById(
        "variableStationTypes_csv_upload"
      ).files;
      console.log(files);
      if (Array.from(files).length < 1) {
        $.notify("Please, submit a file to upload", "warn");
