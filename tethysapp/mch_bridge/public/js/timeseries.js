...
var TIMESERIES_OBJECT = (function () {
   ...
   *************************************************************************/
  var preview_time_series, graph_something, summary_data_load, take_out_starts;
  ...
  summary_data_load = function () {
    ...
    if (summ_obj.length > 0) {
      let warning_et =
        "The MCH instance do not have any variables with data available";
  ...
  preview_time_series = function () {
    ...
    dfd.readCSV(userFile).then((df) => {
      ...
      if (date_index < 0 && val_index < 0) {
        $.notify(
          "Please, the columns containing the date and values should be called Datee and Valuee columns",
          "warn"
        );
